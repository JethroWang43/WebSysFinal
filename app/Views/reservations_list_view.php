    ...
    <div class="card">
        <div class="card-body">
            <?php if (! empty($equipment_items)): ?>
                <?php
                    ...
                    foreach ($equipment_items as $c_ei) {
                        if (! empty($c_ei['category'])) $avail_categories[$c_ei['category']] = true;
                    }
                    ...
                    $totalAvailable = 0;
                    foreach ($equipment_items as $c_ei) {
                        if (isset($c_ei['status']) && strtolower($c_ei['status']) !== 'available') continue;
                        $totalAvailable++;
                        $hay = strtolower(($c_ei['name'] ?? '') . ' ' . ($c_ei['equipment_id'] ?? '') . ' ' . ($c_ei['description'] ?? ''));
                        if ($avail_q !== '' && stripos($hay, strtolower($avail_q)) === false) continue;
                        if ($avail_category !== '' && $avail_category !== 'all' && strtolower(($c_ei['category'] ?? '')) !== strtolower($avail_category)) continue;
                        $availableItems[] = $c_ei;
                    }
                    // Pagination for available items (separate param: avail_page)
                    $availPerPage = 6;
                    $availPage = isset($_GET['avail_page']) ? (int) $_GET['avail_page'] : 1;
                    if ($availPage < 1) $availPage = 1;
                    $availTotalFiltered = count($availableItems);
                    $availPages = max(1, (int) ceil($availTotalFiltered / $availPerPage));
                    if ($availPage > $availPages) $availPage = $availPages;
                ...
                <div class="mb-3">
                    ...
                    <form method="get" class="row g-2 mt-2">
                        <div class="col-md-5">
                            <input type="search" name="avail_q" class="form-control" placeholder="Search available equipment" value="<?= esc($avail_q) ?>" />
                        </div>
                        <div class="col-md-4">
                            <select name="avail_category" class="form-select">
                                <option value="all">All Categories</option>
                                <?php foreach ($avail_categories as $cat => $_): ?>
                                    <option value="<?= esc($cat) ?>" <?= $avail_category === $cat ? 'selected' : '' ?>><?= esc($cat) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-md-3">
                    ...
                    <div class="small text-muted mt-2">Showing <?= $avail_shown ?> of <?= $totalAvailable ?> available items</div>
                </div>

                <table class="table">
                    <thead>
                        <tr><th>Equipment ID</th><th>Equipment</th><th>Category</th><th>Action</th></tr>
                    </thead>
                    <tbody>
                        <?php foreach ($availablePageItems as $ei): ?>
                            <tr>
                                <td><?= esc($ei['equipment_id'] ?? $ei['id']) ?></td>
                                <td><strong><?= esc($ei['name'] ?? '') ?></strong><div class="text-muted small"><?= esc($ei['description'] ?? '') ?></div></td>
                                <td><?= esc($ei['category'] ?? '') ?></td>
                                <td>
                                    <a href="<?= base_url('reservations/create/'.$ei['id']) ?>" class="btn btn-sm btn-outline-primary">Reserve</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                        ...
                        <ul class="pagination pagination-sm">
                            <?php
                                ...
                                for ($p = 1; $p <= $availPages; $p++):
                                    $qs = $baseQs; $qs['avail_page'] = $p;
                                    $link = base_url('reservations') . '?' . http_build_query($qs);
                            ?>
                                <li class="page-item <?= $p == $availPage ? 'active' : '' ?>"><a class="page-link" href="<?= $link ?>"><?= $p ?></a></li>
                            <?php endfor; ?>
                        </ul>
                    </nav>
                <?php endif; ?>
            ...
            <?php else: ?>
                <table class="table">
                    <thead>
                        <tr><th>#</th><th>Equipment</th><th>Name</th><th>ID</th><th>Date Reserved</th><th>Location</th><th>Actions</th></tr>
                    </thead>
                    <tbody>
                        <?php foreach ($reservations as $r): ?>
                            <tr>
                                ...
                                <td>
                                    <a href="<?= base_url('reservations/borrow/'.$r['id']) ?>" class="btn btn-sm btn-primary me-1" onclick="return confirm('Claim and mark this reservation as borrowed?')">Borrow</a>
                                    <a href="<?= base_url('reservations/reschedule/'.$r['id']) ?>" class="btn btn-sm btn-outline-secondary me-1">Reschedule</a>
                                    <a href="<?= base_url('reservations/cancel/'.$r['id']) ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Cancel this reservation?')">Cancel</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                ...
                <nav class="mt-3">
                    <ul class="pagination">
                        <?php
                            $baseQs = [];
                            $baseQs['page'] = 1;
                            $pages = max(1, (int) ceil(($totalFiltered ?? count($reservations ?? [])) / ($perPage ?? 6)));
                            $cur = $page ?? 1;
                            for ($p = 1; $p <= $pages; $p++):
                                $qs = $baseQs; $qs['page'] = $p;
                                $link = base_url('reservations') . '?' . http_build_query($qs);
