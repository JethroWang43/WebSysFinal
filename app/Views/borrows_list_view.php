<div class="py-4">
        ...
        <div class="card-body">
            <div class="mb-3">
                <form method="get" class="row g-2">
                    <div class="col-md-4">
                        <input type="search" name="q" class="form-control" placeholder="Search by ref, equipment, borrower or ID" value="<?= esc($q ?? '') ?>" />
                    </div>
                    <div class="col-md-3">
                        <select name="status" class="form-select">
                            <option value="all">Any</option>
                            <option value="active" <?= isset($status) && $status==='active' ? 'selected' : '' ?>>Active (not due soon)</option>
                            <option value="due_soon" <?= isset($status) && $status==='due_soon' ? 'selected' : '' ?>>Due Soon (≤7 days)</option>
                            <option value="overdue" <?= isset($status) && $status==='overdue' ? 'selected' : '' ?>>Overdue</option>
                        </select>
                    </div>
                    <div class="col-md-3">
                        <input type="search" name="avail_q" class="form-control" placeholder="Search available equipment" value="<?= esc(isset($_GET['avail_q']) ? $_GET['avail_q'] : '') ?>" />
                    </div>
                    <div class="col-md-2">
            ...
            <?php if (! empty($equipment_items)): ?>
                ...
                <div class="mb-3">
                    ...
                    <div class="small text-muted">Showing <?= $avail_shown ?> of <?= $totalAvailable ?> available items</div>
                </div>
                    ...
                    <tbody>
                        <?php foreach ($availablePageItems as $ei): ?>
                            <tr>
                                ...
                                <td>
                                    <a href="<?= base_url('borrowing/create/'.$ei['id']) ?>" class="btn btn-sm btn-outline-success">Borrow</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
            ...
            <?php else: ?>
                <table class="table">
                    <thead>
                        <tr><th>#</th><th>Ref</th><th>Equipment</th><th>Location</th><th>Borrower</th><th>Borrowed</th><th>Due</th></tr>
                    </thead>
                    <tbody>
    ...
    <div class="mt-4 text-muted">For educational purposes only, no copyright infringement intended</div>
</div>
