<div class="py-4">
    ...
    <div class="card p-3 mb-3">
        <h5><?= $equipment['name'] ?? 'Selected item' ?> <small class="text-muted"><?= $equipment['equipment_id'] ?? '' ?></small></h5>
    </div>
    ...
    <form method="post" action="<?= base_url('reservations/submit') ?>">
        ...
        <div class="mb-3">
            ...
            <select name="user_id" id="reserve_user_select" class="form-select">
                ...
                <?php if (! empty($users)): foreach ($users as $u): ?>
                    <option value="<?= $u['id'] ?>"><?= esc($u['fullname'] ?: $u['username']) ?></option>
        ...
        <div class="mb-3">
            ...
            <input class="form-control" name="name" id="reserve_name" list="reserve_students" autocomplete="off" />
        ...
        <div class="mb-3">
            ...
            <input class="form-control" name="id_number" id="reserve_id_number" placeholder="202311220" />
        </div>

        <div class="mb-3">
            <label class="form-label">Bring To (Location / Room)</label>
            <input class="form-control" name="use_location" id="reserve_use_location" placeholder="e.g. Lab A - Room 101" />
        </div>
        <div class="row">
            <div class="col-md-6 mb-3">
                <label class="form-label">Reservation Date</label>
                <input class="form-control" type="date" name="reserve_date" value="<?= date('Y-m-d') ?>" />
            </div>
            <div class="col-md-6 mb-3">
                <label class="form-label">Reservation Time</label>
                <input class="form-control" type="time" name="reserve_time" value="<?= date('H:i') ?>" />
            </div>
        </div>

        <div class="mb-3">
            <label class="form-label">Due Date (return by)</label>
            <input class="form-control" type="date" name="due_date" value="<?= date('Y-m-d', strtotime('+7 days')) ?>" />
...
// Helper to find user id by exact name (case-insensitive)
function findUserIdByName(name) {
    name = (name || '').toLowerCase().trim();
    for (const k in usersMeta) {
        if ((usersMeta[k].name || '').toLowerCase() === name) return usersMeta[k].id;
    }
    return null;
}

document.getElementById('reserve_user_select')?.addEventListener('change', function(e) {
...
document.getElementById('reserve_name')?.addEventListener('input', function(e) {
    const val = this.value;
    const uid = findUserIdByName(val);
    if (uid) {
        const meta = usersMeta[uid] || {};
        document.getElementById('reserve_id_number').value = meta.studentId || '';
        // also set the select to match
